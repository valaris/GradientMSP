    ...
    Process {
        try {
            ...
            foreach ($record in $usageRecords.Values) {
                Write-Host "$($record.name) | $($record.id) has $($record.usage) seats used for $($record.service), attempting to create billing request..." -ForegroundColor Green

                try {
                    $body = Initialize-PSCreateBillingRequest -ClientOId $null -AccountId $record.id -UnitCount $record.usage
